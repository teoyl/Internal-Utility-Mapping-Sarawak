...
public class MoalAction extends BaseActionSupport<MoalModel> implements ModelDriven<MoalModel> {
...
//            closeSession();
    ...
    public String dynamicUpdate() {
        if (dcRecordIds != null && dcRecordIds.length > 0) {
            ...
            MoalModel errLog = new MoalModel();
            String[] updatingColumn = new String[] {"log_remark", "updated_by", "updated_date"};
            try {
                baseDAO.beginBatchTransaction();
                for (String recordId : dcRecordIds) {
                    errLog.setID(recordId);
                    errLog.defaultUpdateProperties();
                    errLog.setLog_remark(request.getParameter("log_remark"+recordId));
