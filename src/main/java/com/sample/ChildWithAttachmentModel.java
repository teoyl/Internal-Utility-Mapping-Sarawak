...
@Table(name="t_uppy")
@NamedQueries({
    @NamedQuery(name = "ChildWithAttachmentModel.findBy_createdBy",query = "from ChildWithAttachmentModel where created_by = :createdBy"),
    @NamedQuery(name = "ChildWithAttachmentModel.findBy_drDocId",query = "from ChildWithAttachmentModel where dr_doc_id = :drDocId"),
    @NamedQuery(name = "ChildWithAttachmentModel.findBy_uppyFileDesc",query = "from ChildWithAttachmentModel where uppy_file_desc = :uppyFileDesc")
})
public class ChildWithAttachmentModel extends ModelBase implements java.io.Serializable {
    ...
    public ChildWithAttachmentModel() {
        getUppyUpload_appCodeSetup().put("moreAttachmentModel_file", "dr_doc_id;childWithAttachmentActualFolder");
        getUppyUpload_appCodeSetup().put("moreAttachmentModel_file2", "dr_doc_id_2;childWithAttachmentActualFolder");
...
//            getMoveToActualPathMap().put("moreAttachmentModel_file", "dr_doc_id;childWithAttachmentActualFolder");
//            getMoveToActualPathMap().put("moreAttachmentModel_file2", "dr_doc_id_2;childWithAttachmentActualFolder");
//            this.set_uppyUploadFile_drDocPath("childWithAttachmentActualFolder");
//            if (Validator.notEmpty(get_uppyUploadFile_drDocId())) {
//                this.set_uppyUploadFile_drDocId(get_uppyUploadFile_drDocId() + ","+dr_doc_id);
//            } else {
...
//            }
    ...
    public void setDr_doc_id_2(String dr_doc_id_2) {
        if (Validator.notEmpty(dr_doc_id_2)) {
//            getMoveToActualPathMap().put("dr_doc_id_2", "childWithAttachmentActualFolder");
//            this.set_uppyUploadFile_drDocPath("childWithAttachmentActualFolder");
//            if (Validator.notEmpty(get_uppyUploadFile_drDocId())) {
//                this.set_uppyUploadFile_drDocId(get_uppyUploadFile_drDocId() + ","+dr_doc_id_2);
//            } else {
//                this.set_uppyUploadFile_drDocId(dr_doc_id_2);
//            }
        }
    ...
    public String getUppy_file_desc() {
        uppy_file_desc = "_moreAttachment_";
    ...
    public void postInsert(Session session) throws Exception {
        System.out.println("this.uppyUploadFile_drDocId = " + this.get_uppyUploadFile_drDocId());
    ...
    public void preDelete(Session session, Object deletingObject) throws Exception {
        if (this.getDrDocRepoModel() != null) {
            BaseDAOImpl dao = new BaseDAOImpl();
            dao.setSession(session);
            SFTPBean sftp = new SFTPBean();
            if (this.getDrDocRepoModel() != null) {
                sftp.deleteFile(this.getDrDocRepoModel().getDr_doc_path());
                if (this.appendRecordID()) {
                    if (sftp.isDirExists(this.getDrDocRepoModel().getDr_doc_path().substring(0, this.getDrDocRepoModel().getDr_doc_path().lastIndexOf("/")), Boolean.FALSE)) {
                        sftp.deleteFolder(this.getDrDocRepoModel().getDr_doc_path().substring(0, this.getDrDocRepoModel().getDr_doc_path().lastIndexOf("/")));
                    }
                }
//                System.out.println("this.getDrDocRepoModel().getDr_doc_path() =  " + this.getDrDocRepoModel().getDr_doc_path());
                dao.deleteModel(this.getDrDocRepoModel());
            }
            if (this.getDrDocRepoModel2() != null) {
                sftp.deleteFile(this.getDrDocRepoModel2().getDr_doc_path());
                if (this.appendRecordID()) {
                    if (sftp.isDirExists(this.getDrDocRepoModel2().getDr_doc_path().substring(0, this.getDrDocRepoModel2().getDr_doc_path().lastIndexOf("/")), Boolean.FALSE)) {
                        sftp.deleteFolder(this.getDrDocRepoModel2().getDr_doc_path().substring(0, this.getDrDocRepoModel2().getDr_doc_path().lastIndexOf("/")));
                    }
                }
                dao.deleteModel(this.getDrDocRepoModel2());
            }
        }
    }
}
