...
Viewer.prototype._createContainer = function(options) {
  ...
  if (isString(parent)) {
    ...
    if (parent === null) {
      parent = document.getElementById(temp);
    }
  }
...
function BpmnRenderer(eventBus, styles, pathMap, priority) {
  ...
  function initMarkers(svg) {
    ...
    createMarker('sequenceflow-end-completed', {
      element: svg.path('M 1 5 L 11 10 L 1 15 Z'),
      ref: { x: 11, y: 10 },
      scale: 0.5,
      attrs: {
  ...
  function drawCircle_completed(p, width, height, offset, attrs) {
...
//      stroke: '#999900',
  ...
  function drawDiamond_pending(p, width, height, attrs) {
  ...
  function drawDiamond_completed(p, width, height, attrs) {
  ...
  var handlers = this.handlers = {
    'bpmn:Event': function(p, element, attrs) {
        if (containCompleteKey(element.id)) {
            return drawCircle_completed(p, element.width, element.height,  attrs);
        } else {
    ...
    'bpmn:Activity': function(p, element, attrs) {
        ...
        if (element.id === currentTaskBpmnId) {
            attrs = computeStyle(attrs, {
                'stroke-dasharray':'8, 8'
            });
        }
        
        if (containPendingKey(element.id)) {
            attrs = computeStyle(attrs, {
                fill: '#ffaf4d'
            });
            
//            return drawRect_pending(p, element.width, element.height, TASK_BORDER_RADIUS, attrs);
        } else if (containInProgressKey(element.id)) {
            attrs = computeStyle(attrs, {
//                fill: '#87CEFA'
                fill: '#cde9f2'
            });
//            return drawRect_inProgress(p, element.width, element.height, TASK_BORDER_RADIUS, attrs);
        } else if (containCompleteKey(element.id)) {
            attrs = computeStyle(attrs, {
                fill: '#c1eaa6'
            });
//            return drawRect_completed(p, element.width, element.height, TASK_BORDER_RADIUS, attrs);
        }
...
//        fillOpacity: 0.95,
    ...
    'bpmn:Lane': function(p, element, attrs) {
      ...
      if (semantic.$type === 'bpmn:Lane') {
        ...
        renderLaneLabel(p, text, element);
        var rect2 = drawRect(p, 30, element.height, 0, attrs || {
            fill: 'none'
        });
      }
      return rect;
    ...
    'bpmn:ExclusiveGateway': function(p, element) {
        if (containCompleteKey(element.id)) {
            var diamond = drawDiamond_completed(p, element.width, element.height);
        } else if (containPendingKey(element.id)) {
            var diamond = drawDiamond_pending(p, element.width, element.height);
        } else {
            var diamond = drawDiamond(p, element.width, element.height);
        }
    ...
    'bpmn:Gateway': function(p, element) {
        if (containCompleteKey(element.id)) {
            return drawDiamond_completed(p, element.width, element.height);
        } else if (containPendingKey(element.id)) {
            return drawDiamond_pending(p, element.width, element.height);
        } else {
    ...
    'bpmn:SequenceFlow': function(p, element) {
      ...
      if (containCompleteKey(element.id) || containPendingKey(element.id) || containInProgressKey(element.id)) {
        var path = drawPath(p, pathData, {
          strokeLinejoin: 'round',
          stroke: 'green',
          markerEnd: marker('sequenceflow-end-completed')
        });
      } else {
        var path = drawPath(p, pathData, {
          opacity: 0.5,
...
function openTag (parser, selfClosing) {
  ...
  if (!selfClosing) {
    ...
    if (!parser.noscript && parser.tagName.toLowerCase() === "theScript") {
...
GraphicsFactory.prototype._createContainer = function(type, parentGfx) {
  ...
  // create visual
    if (parentGfx.toString().indexOf("Id_aab1f8e7-f6b5-4f33-9647-678e0df61f05") <= 0) {
        if (found) {
...
module.exports.createLine_completed = function(points, attrs) {
  return Snap.create('polyline', { points: toSVGPoints(points) }).attr(attrs || {});
};
