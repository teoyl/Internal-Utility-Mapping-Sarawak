...
<%int tableCol = 0;%>
    <div class="table-responsive">
        <table class="table table-sds table-sm fs--1 table-striped table-hover " border="0" cellpadding="1" cellspacing="1" width="100%">
            <!-- display the columns -->
                ...
                <th style="width:1%;" class="align-middle white-space-nowrap">
                    <div class="form-check fs-0 mb-0">
                        <s:if test="getDynamicResult(list_param).size() > 0">
                            <input type="checkbox"  id="cbselect" class="selectAll form-check-input" name="cbselect" onClick="toggleCheckboxByName(this,'${list_param}_selected');">
                ...
                <s:iterator value="getDynamicResult(list_param)" status="resultStatus" var="r">
                    <tr>
                        <td class="align-middle white-space-nowrap <s:if test="#resultStatus.odd == true ">odd</s:if><s:else>even</s:else>" width="1%">
                            <div class="form-check fs-0 mb-0">
                                <input type="checkbox" name="${list_param}_selected" class="checkbox_child form-check-input" id="${list_param}_ids_${resultStatus.index}" value="<s:property value="%{getDynamicResultPrimaryKey(list_param,#resultStatus.index)}"/>" onclick="toggleSelectAll()">
                                <label for="${list_param}_ids_${resultStatus.index}" class="tableListingCheckbox form-check-label"></label>
                            </div>
                        ...
                        <s:iterator value="getDynamicDisplayField(list_param)" var="displayCol">
                            <td class="align-middle white-space-nowrap <s:if test="#resultStatus.odd == true ">odd</s:if><s:else>even</s:else>" id="${displayCol}" style="word-wrap: break-word; <s:property value='%{getFieldStyleFormat(#displayCol, "detail", #r.get(#displayCol), #r)}'/>">
        ...
        <%-- if no results --%>
        <s:if test="getDynamicResult(list_param).size() <= 0">
            <tr><td class="text-center" colspan="<%=tableCol + 1%>"><s:text name="common.noRecordFound" /></td></tr>
            </s:if>
...
<s:if test="getDynamicResult(list_param).size() > 0">
    <div class="fs--1">
            <s:include value="../pagination/dynamicListPaging.jsp">
