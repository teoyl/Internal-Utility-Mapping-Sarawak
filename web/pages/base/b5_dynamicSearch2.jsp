...
<s:iterator value="getDynamicSearchField(list_param)" var="field" status="rowStatus">
    ...
    <s:else>
        <div class="row mb-1">
            <div class="col-md-4 col-form-label col-form-label-sm"><s:property value="%{getDynamicSearchLabel(list_param)[#rowStatus.index]}"/></div>
            <div class="col-md-8">
                ...
                <s:if test='#field.startsWith("_date_")'>
                    <s:if test='#field.endsWith("_fromTo")'>
                        <s:textfield readonly="true" theme="simple" name="search_%{#field.substring(6, (#field.length() - 7))}From" size="26" value='%{searchFieldsDateData.get("search_"+extractSearchFieldForDate(#field)+"From")}' cssClass="form-control form-control-sm"/>
                        ...
                        <s:textfield readonly="true" theme="simple" name="search_%{#field.substring(6, (#field.length() - 7))}To" size="26" value='%{searchFieldsDateData.get("search_"+extractSearchFieldForDate(#field)+"To")}' cssClass="form-control form-control-sm"/>
                    ...
                    <s:else>
                        <s:textfield readonly="true" theme="simple" name="search_%{#field.substring(6)}" size="26" value='%{searchFieldsDateData.get("search_"+extractSearchFieldForDate(#field))}' cssClass="form-control form-control-sm"/>
                ...
                <s:elseif test='#ddMap.get(#searchfield_dd_key) != null'>
                    ...
                    <s:else>
                        <s:if test="#ddMap.get(#searchfield_dd_list).equalsIgnoreCase('setupcode')">
                            <s:select theme="simple"  cssClass="form-control form-control-sm"  name="search_%{#field}" list="#ddMap.get(#searchfield_dd)" listKey="code_id" listValue="code_desc" value="%{getDynamicSearchData(list_param)[#rowStatus.index]}" />
                        </s:if>
                        <s:else>
                            <s:select theme="simple" cssClass="form-control form-control-sm" name="search_%{#field}" list="#ddMap.get(#searchfield_dd)" listKey="keyData" listValue="valueData" value="%{getDynamicSearchData(list_param)[#rowStatus.index]}" />
                        </s:else>
                    </s:else>
                </s:elseif>
                <s:elseif test='#lookupMap.get(#searchfield_lookupSearch) != null'>
                    <s:textfield theme="simple" name="search_%{#field}" value="%{getDynamicSearchData(list_param)[#rowStatus.index]}" size="30" cssStyle="padding: 1px 3px 1px 3px; %{getFieldStyleFormat(#field, 'searchField', '')" cssClass="form-control form-control-sm"/>
                    <script language="javascript">
