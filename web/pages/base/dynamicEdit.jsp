...
function required(){
    <s:property escape="false" value="editPageRequired"/>
}
...
<body>
	...
	<form theme="simple" id="dynamicFormId" action="processUpdateDynamic" method="post">
		...
		<s:hidden name="objId" value="%{getFieldData('objId')}"/>
                <table cellspacing="0" cellpadding="2" border="0" width="100%" class="form">
                    <tr>
                        ...
                        <td align="right">
                            <s:submit type="button" cssClass="defaultButton" theme="simple" action="processUpdateDynamic" value='%{getText("button.save")}' onclick="return localValidateForm(this.form, 'update')"/>
                ...
                <table><s:iterator value="pageLabels" var="field" status="rowStatus">
                    <tr>
                        <td width="20px">&nbsp;</td>
                        <td width="${labelWidth}px" align="left">${pageLabelsDesc[rowStatus.index]}<s:if test='isRequiredField("update", #field)'><jsp:include page="/pages/base/requiredField.jsp"/></s:if></td>
                        <td width="3px">:</td>
                        <td align="left"><s:set name="itemText" value="%{pageFields[#rowStatus.index]}"/>
                            ...
                            <s:elseif test='#itemText.startsWith("checkbox:")'>
                                <s:checkbox theme="simple" name="%{getNameOfColumn(#field)}" fieldValue="Y" value='%{getFieldData(#itemText).equals("Y")?"True":"false"}' />
                            </s:elseif>
                            <s:else>
