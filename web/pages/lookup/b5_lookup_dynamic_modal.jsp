<%@taglib uri="/struts-tags" prefix="s"%>
    <div class="row mb-1">
        <div class="col-md-12">
            ...
            <s:iterator value="searchFields" var="field" status="rowStatus">
                ...
                <s:else>
                    <div class="row mb-1">
                        <div class="col-md-4 col-form-label col-form-label-sm">${searchFieldsLabel[rowStatus.index]}<s:if test='isRequiredField(#field)'><jsp:include page="/pages/base/requiredField.jsp"/></s:if></div>
                        <div class="col-md-8">
                            ...
                            <s:if test='#field.startsWith("_date_")'>
                                <s:if test='#field.endsWith("_fromTo")'>
                                    <div class="input-group input-group-sm" id="${field.substring(6, (field.length() - 7))}_DateRange">
                                ...
                                <s:else>
                                    <div class="input-group input-group-sm lookup_date">
                                        <div class="input-group-text input-button" data-toggle>
                                            <i class="fa fa-calendar"></i>
                                        </div>
                                        <input type="text" class="form-control form-control-sm" theme="simple" name="search_<s:property value='%{#field.substring(6)}'/>" value='<s:property value='%{searchFieldsDateData.get("search_"+extractSearchFieldForDate(#field))}'/>' data-input/>
                                    </div>
                                    <%--<div class="form-group form-group-default">
                            ...
                            <s:elseif test='getSearchFieldDD().get(#searchfield_dd_key) != null'>
                                ...
                                <s:else>
                                    <s:if test="getSearchFieldDD().get(#searchfield_dd_list).equalsIgnoreCase('setupcode')">    <%--Added by Delvene @ 29-Aug-2013--%>
                                        <s:select theme="simple" cssClass="dsrfdd rowText form-control form-control-sm sds-dropdown mySelectBox input-sm" name="search_%{#field}" list="getSearchDDList(#searchfield_dd)" listKey="code_id" listValue="code_desc" value="%{searchFieldsData[#rowStatus.index]}" />
                                    </s:if>
                                    <s:else>
                                        <s:select theme="simple" cssClass="dsrfdd rowText form-control form-control-sm sds-dropdown mySelectBox input-sm" name="search_%{#field}" list="getSearchDDList(#searchfield_dd)" listKey="keyData" listValue="valueData" value="%{searchFieldsData[#rowStatus.index]}" />
                                    </s:else>
                                </s:else>
                            </s:elseif>
                        ...
                        <div class="col-md-8">
                            ...
                            <s:elseif test='getSearchFieldDD().get(#searchfield_dd_key) != null'>
                                ...
                                <s:else>
                                    ...
                                    <s:else>
                                        <s:select theme="simple" cssClass="dsrfdd rowText form-control form-control-sm sds-dropdown mySelectBox" name="search_%{#field}" list="getSearchDDList(#searchfield_dd)" listKey="keyData" listValue="valueData" value="%{searchFieldsData[#rowStatus.index]}" />
                                    </s:else>
                                </s:else>
                            </s:elseif>
                            <s:elseif test='getSearchFieldLookup().get(#searchfield_lookupSearch) != null'>
            ...
            <s:if test='showPageSize && !(useDataTable)'>
                <div class="row">
                    <div class="col-md-4 col-form-label col-form-label-sm"><s:text name="paging.recordPerPage"/></div>
                    <div class="col-md-8">
                        <s:select theme="simple" id="mainPageSize" name="mainPageSize" list="pageSizeOption" listKey="keyData" listValue="valueData" value="%{pageSize}" cssClass="dsrfdd rowText form-control form-control-sm sds-dropdown mySelectBox"/>
                    </div>
                </div>
            </s:if>
        </div>
    ...
    <div class="row">
        <div class="col-md-12 text-center">
            <button class="btn btn-primary btn-sm" type="submit" onclick="getPageSize('', this.form); submitLookupForm('mSearchLookup', 'mSearchLookup', 'lookupModal'); return false;"><i class="fa fa-search"></i>&nbsp;<s:text name="button.search"/></button>
            <button class="btn btn-falcon-default btn-sm reset" type="button" onclick="resetFields(this.form)"><i class="fa fa-undo"></i>&nbsp;<s:text name="button.reset"/></button>
            <s:if test="hasMoreSearchField">
                <button class="btn btn-falcon-default btn-sm showHide" type="button" onclick="return showHideLookupMoreField();"><i class="showHideIcon fa <s:if test='showHideLookupMoreField_.equals("H")'>fa-arrow-down</s:if><s:else>fa-arrow-down</s:else> search"></i><s:text name="button.more"/></button>
            </s:if>
