...
//                        {
                            ...
                            if (document.getElementById(parentFormId)[writeToValue])
                            {
                                    ...
                                    var theTarget = document.getElementById(parentFormId)[writeToValue];
                                    $( theTarget ).change();
                                    document.getElementById(parentFormId)[writeToValue]
                            } else
                            {
                                $("#"+writeToValue).html(retVal);
                            }
//                        }
                    }
...
//                            escCnt = 0;
            ...
            function Navi(e) {
                ...
                <%--${showHideLookupMoreField_}--%>
                    ...
                    $('.lookup_date').flatpickr({
                        wrap: true,
                        dateFormat: newDateRangePicker_defaultFormat,
                    });
                });
                function submitLookupForm(submitAction, formId, itemDiv) {
                    var errors = new Array();
                    validateLookupRequired(document.getElementById(formId), errors);
                    if (errors.length > 0) {
                        alert(errors.join('\n'));
                        setFocus(document.getElementById(formId));
                        return false;
                    } else {
                        divSubmitForm(submitAction, formId, itemDiv);
                        return true;
                    }
            ...
            <form action="mSearchLookup" id="mSearchLookup" method="post">
                ...
                <s:if test="!hideSearchScreen">
                    ...
                    <% } else {%>
                    <jsp:include page='b5_lookup_dynamic_modal.jsp'/>
                    <% } %>
                </s:if>
                <div class="table-responsive mt-3">
                    <table class="table table-sds table-sm fs--1 table-striped table-hover" width="100%">
                        <thead class="bg-200 text-900">
                            ...
                            <tr class="CLASS_TABLE_HEADER" width="2%">
                                ...
                                <s:iterator value="goodDisplayingColumns" status="columnStatus" var="column">
                                    ...
                                    <th class="align-middle white-space-nowrap" style="<s:property value='%{getFieldStyleFormat(displayingColumns[columnStatus.index])}'/>"><%tabelCol++;%>
                        ...
                        <s:if test="result.size() > 0">
                            ...
                            <s:iterator value="result" status="resultStatus" var="r">
                                <tr style="cursor: pointer" class="<s:if test="#resultStatus.odd == true ">lk_odd</s:if><s:else>lk_even</s:else>" id="resultTR_${resultStatus.index}" onclick="updateOpener(this)">
                                    <td class="align-middle white-space-nowrap" width="1%">${rowIndex + resultStatus.index}</td>
                                    <s:iterator value="displayingColumns" var="displayCol">
                                        ...
                                        <%--<s:iterator value="r" status="aa">--%>
                                            <%--<s:if test="#resultMap.key.equals(#displayCol)">--%>
                                                <!--<td id="${resultMap.key}" width="250">-->
                                                <td class="align-middle white-space-nowrap" id="${displayCol}" width="250">
                                                    
                                                    <span>
                                                        <s:if test='getCheckFormatField(#displayCol).equals("true")'>
                                                            <s:property value="%{getFormattedField(#displayCol.toUpperCase(), #resultStatus.index)}"/>
                                                        </s:if><s:else>
                                                            <s:property value="%{#r.get(#displayCol.toUpperCase())}"/></s:else>
                                                        </span>
                                                    <s:iterator value="hiddenColumns" var="hiddenCol">
                                                        <s:if test='getCheckFormatField(#displayCol.toUpperCase()).equals("true")'>
                                                            <input type="hidden" name="${hiddenCol}" id="${hiddenCol}" value="<s:property value="%{getFormattedField(#displayCol.toUpperCase(), #resultStatus.index)}"/>"/>
                                                        </s:if><s:else>
                                                            <input type="hidden" name="${hiddenCol}" id="${hiddenCol}" value="<s:property value="%{getHiddenValue(#resultStatus.index, #hiddenCol.toUpperCase())}"/>"/>
                                                        </s:else>
                                                    </s:iterator>
                                                </td>
                                            <%--</s:if>--%>
                        ...
                        <s:if test="result.size() <= 0 && searched">
                            <tr><td class="remark align-middle white-space-nowrap" colspan="<%=tabelCol + 1%>"><s:text name="common.noRecordFound" /></td></tr>
                            </s:if>
                            <%-- pagination --%>
                            <s:if test="result.size() > 0 && searched">
                            <tr><td colspan="<%=tabelCol + 1%>">
                                    <jsp:include page="../pagination/b5_paging_modal.jsp"></jsp:include>
                                    </td></tr>
                            </s:if>
                    </table>
                </div>
