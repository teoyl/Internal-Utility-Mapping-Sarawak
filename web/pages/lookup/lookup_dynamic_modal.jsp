<%@taglib uri="/struts-tags" prefix="s"%>
                            ...
                            <s:elseif test='getSearchFieldDD().get(#searchfield_dd_key) != null'>
                                ...
                                <s:else>
                                    <s:if test="getSearchFieldDD().get(#searchfield_dd_list).equalsIgnoreCase('setupcode')">    <%--Added by Delvene @ 29-Aug-2013--%>
                                        <s:select theme="simple" cssClass="dsrfdd rowText form-control sds-dropdown mySelectBox input-sm" name="search_%{#field}" list="getSearchDDList(#searchfield_dd)" listKey="code_id" listValue="code_desc" value="%{searchFieldsData[#rowStatus.index]}" />
                                    </s:if>
                                    <s:else>
                                        <s:select theme="simple" cssClass="dsrfdd rowText form-control sds-dropdown mySelectBox input-sm" name="search_%{#field}" list="getSearchDDList(#searchfield_dd)" listKey="keyData" listValue="valueData" value="%{searchFieldsData[#rowStatus.index]}" />
                                    </s:else>
                                </s:else>
                            </s:elseif>
    ...
    <div class="row">
        <div class="col-md-12 text-center">
            <button class="btn btn-primary" type="submit" onclick="getPageSize('', this.form); submitLookupForm('mSearchLookup', 'mSearchLookup', 'lookupModal'); return false;"><i class="fa fa-search"></i>&nbsp;<s:text name="button.search"/></button>
            ...
            <s:if test="hasMoreSearchField">
                <button class="btn btn-default showHide" type="button" onclick="return showHideLookupMoreField();"><i class="showHideIcon fa <s:if test='showHideLookupMoreField_.equals("H")'>fa-arrow-down</s:if><s:else>fa-arrow-down</s:else> search"></i><s:text name="button.more"/></button>
            </s:if>
