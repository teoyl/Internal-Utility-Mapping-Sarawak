        ...
        <jsp:include page="/pages/base/actionError.jsp"></jsp:include>
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4><s:property value="%{modelList[0].system_descs}" /> <small>[<s:text name="button.edit"/>]</small></h4>
                </div>
                                    ...
                                    <s:if test='#paramItem.field_indicator.equals("input")'>
                                        ...
                                        <s:if test='#paramItem.numericJavascript == null || #paramItem.numericJavascript.equals("")'>
                                            <div class="row form-group">
                                                <label class="col-md-4 control-label sub"><s:property value="#paramItem.parameter_descs"/><s:if test='#paramItem.field_indicator.equalsIgnoreCase("input") && #paramItem.req_field.equalsIgnoreCase("Y")'><jsp:include page="/pages/base/requiredField.jsp"/></s:if></label>
                                                    <div class="col-md-6">
                                                    <s:textfield cssClass="form-control" theme="simple" id="modelList%{#theIndex}_parameter_value" maxlength="%{#paramItem.textFieldWidth}" name="modelList[%{#theIndex}].parameter_value" value="%{#paramItem.parameter_value}"/>
                                                </div>
                                            </div>
                                        </s:if><s:else>
                                            <div class="row form-group">
                                                <label class="col-md-4 control-label sub"><s:property value="#paramItem.parameter_descs"/><s:if test='#paramItem.field_indicator.equalsIgnoreCase("input") && #paramItem.req_field.equalsIgnoreCase("Y")'><jsp:include page="/pages/base/requiredField.jsp"/></s:if></label>
                                                    ...
                                                    <s:if test='#paramItem.numericJavascript.startsWith("byClass_")'>
                                                        <input class="form-control <s:property escapeHtml="false" value="#paramItem.numericJavascript.substring(8)"/>" type="text" id="modelList${theIndex}_parameter_value" maxlength="${paramItem.textFieldWidth}" name="modelList[${theIndex}].parameter_value" value="${paramItem.parameter_value}"/>
                                                    </s:if><s:else>
                                                        <input class="form-control" <s:property escapeHtml="false" value="#paramItem.numericJavascript"/> type="text" id="modelList${theIndex}_parameter_value" maxlength="${paramItem.textFieldWidth}" name="modelList[${theIndex}].parameter_value" value="${paramItem.parameter_value}"/>
                                                    </s:else>
                                                </div>
                                            </div>
                                        </s:else>
            ...
            <div class="row form-row-margin">
                <div class="col-md-12 text-right login">
                    <s:submit type="button" cssClass="btn btn-default" theme="simple" action="loadAddPageParameter" value="Copy This Parameter"/>
                    <s:submit type="button" cssClass="btn btn-primary" theme="simple" action="processUpdateParameter" value="Save" onclick="return localValidateForm(this.form);"/>
                    <s:submit type="button" cssClass="btn btn-default" theme="simple" action="cancelParameter" value="Cancel"/>
                </div>
            </div>
        </form>
    </body>
