...
<form method="post" id="postReportFormID" action="processUpdatePost">
    ...
    <div class="xbox">
        <table width="100%" cellpadding="0" cellspacing="0" border="0" class="pr-list">
            ...
            <tbody>
                <s:iterator value="systemUserReportList" var="systemUserReports" status="systemUserReportStatus">
                    <tr class="<s:if test="#systemUserReportStatus.odd == true ">odd</s:if><s:else>even</s:else>">
                        <td align="center">${systemUserReportStatus.index + 1}</td>
                        <td><%--some space--%></td>
                        <td>
                            <s:if test="(#systemUserReports.reportAccessRight)">
                                <a href="dynamicRptAction?rptCode=${systemUserReports.reportCode}">${systemUserReports.reportName}</a>
                            </s:if>
                            <s:else>
                                ${systemUserReports.reportName}
                            </s:else>
                        </td>
                    </tr>
                </s:iterator>
