...
//                    ],
                    ...
                    "columnDefs": [ 
                        ...
                        "render": function ( data, type, row, meta ) {
                            info = table.page.info();
                            $("#divTemp").html($("#divSelect").html());
                            $("#divTemp").find("select").attr("id", "id"+(info.page * info.length) + (meta.row+1)+"aa");
                            $("#divTemp").find("select").attr("name", "name"+(info.page * info.length) + (meta.row+1));
                            $("#divTemp").find("select").addClass("sds-dropdown");
                            return $("#divTemp").html();
...
<%--                    <s:iterator value="recordList" var="rec" status="recStatus">
        ...
        <div id="divSelect" class="hidden">
            <s:select id="divSelectId" cssClass="form-control" list="sampleList" listKey="keyData" listValue="valueData"/>
        </div>
        <input type="button" onclick="resetSelect2()" value="Reset Select2"/>
        <form id="dtFormId">
