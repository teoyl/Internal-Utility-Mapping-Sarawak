<%-- 
    Document   : editChecklistSetup
    Created on : Jan 10, 2019, 3:57:38 PM
    Author     : User
--%>
    ...
    <head>
        ...
        <script nonce="r4DjhKbfO5ry">    
            $( document ).ready(function() {
                ...
                $("#checklist_status").select2();
                
                $('[data-toggle=expiry_date]').each(function() {
                    var target = $(this).data('target-name');
                    var t = $('#' + target);
                    t.datepicker({
                      dateFormat: 'd M yy',
                      onSelect: function(selected) {
                        $("#effective_date").datepicker("option","maxDate", selected)
                      }
                    });
                ...
                $('[data-toggle=effective_date]').each(function() {
                    var target = $(this).data('target-name');
                    var t = $('#' + target);
                    t.datepicker({
                      dateFormat: 'd M yy',
//                      numberOfMonths: 2,
                      onSelect: function(selected) {
                        $("#expiry_date").datepicker("option","minDate", selected)
                      }
                    });
                    $(this).on("click", function() {
                      t.datepicker("show");
                 ...
                 $("#processDeleteChecklistItemsChecklistSetup").on("click", function(e) {
                     e.preventDefault();
                     var checklistForm = this.form;
                     if (isCheckboxSelected(checklistForm.list_selected)) {
                        if(confirmDelete()) {
                            checklistForm.action = "processDeleteChecklistItemsChecklistSetup";
    ...
    <body>
        <div class="x_panel">
            ...
            <form method="post" id="addChecklistFormId" action="">
                <s:hidden theme="simple" name="_deletedItem" />
                <s:hidden theme="simple" name="checklist_id" value="%{model.checklist_id}"/>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 text-end">
                        <button class="btn btn-primary" type="submit" name="action:processUpdateChecklistSetup" id="processInsertChecklistSetup"  onclick="return validateForm(this.form, 'insert')"><i class="fa fa-save"></i> <s:text name="button.save" /></button>        
                        <button class="btn btn-default" type="submit" name="action:cancelChecklistSetup" id="cancelChecklistSetup"><i class="fa fa-times"></i> <s:text name="button.cancel" /></button>
                    </div>    
                </div>
                ...
                <div class="row">
                    <div class="col-md-12 form-row-margin">
                        ...
                        <s:if test="has_right('processDeleteChecklistItems')">
                            <button class="btn btn-default" type="submit" name="action:processDeleteChecklistItemsChecklistSetup" id="processDeleteChecklistItemsChecklistSetup"><i class="fa fa-trash"></i> <s:text name="utimaps.setup.checklist.deleteItems" /></button>
                        </s:if>
                    </div>
                </div>    
                                    ...
                                    <thead>
                                        <tr>
                                            <th width="1%" align="center">
                                                <s:if test="model.checklistItemList.size() > 0">
                                                    <input type="checkbox" id="list_select" name="list_select" onclick="toggleCheckbox(this, list_selected);">
                                                </s:if>
                                                <s:else>
                                    ...
                                    <tbody>
                                        <s:iterator value="model.checklistItemList" status="listStatus" var="checklistItem">
                                            <tr class="<s:if test="#listStatus.odd == true ">odd</s:if><s:else>even</s:else>">
                                                ...
                                                <td align="center">
                                                    <s:checkbox theme="simple" name="list_selected" id="list_selected" fieldValue="%{#listStatus.index}" onclick="checkToggleCheckbox(list_select, delList_ids)"/>
                                                </td>
                                                <td width="600">
