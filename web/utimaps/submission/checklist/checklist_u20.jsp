<%-- 
    Document   : checklist_u20
    Created on : Jun 12, 2024, 2:22:52 PM
    Author     : yonglai
--%>
...
<h5 class="p-1 border-bottom border-bottom-lg-1 bg-secondary text-white mb-0"><s:text name="utimaps.checklist.u20" /></h5>
<div class="table-responsive scrollbar" >
    <table class="table table-hover table-striped overflow-hidden " id="checklist_table">
        ...
        <tbody>
            <s:if test="model.u20ChecklistSetupModel.checklistItemList.size() > 0">
                <s:iterator value="model.u20ChecklistSetupModel.checklistItemList" status="listStatus" var="checklistItem">
                    <tr class="<s:if test="#listStatus.odd == true ">odd</s:if><s:else>even</s:else>">
                        <s:hidden name="model.u20ChecklistSetupModel.checklistItemList[%{#listStatus.index}].checklistResultList[0].cl_id" value="%{#checklistItem.checklistResultList[0].cl_id}" />
                        <s:hidden name="model.u20ChecklistSetupModel.checklistItemList[%{#listStatus.index}].checklistResultList[0].case_id" value="%{model.job_id}" />
                        <s:hidden name="model.u20ChecklistSetupModel.checklistItemList[%{#listStatus.index}].checklistResultList[0].check_id" value="%{model.u20ChecklistModel.check_id}" />
                        <s:hidden name="model.u20ChecklistSetupModel.checklistItemList[%{#listStatus.index}].checklistResultList[0].ci_id" value="%{#checklistItem.ci_id}" />
                        <td colspan="2" class="col-md-3">
                            <div class="d-block">
                                <s:hidden name="model.u20ChecklistSetupModel.checklistItemList[%{#listStatus.index}].checklistResultList[0].ci_desc" value="%{#checklistItem.ci_desc}" />
                            ...
                            <div class="d-flex align-items-center mt-1">
                                ...
                                <div class="col-md-9">
                                    <p class="checklist-file text-truncate max-w-200">
                                        <s:hidden name="model.u20ChecklistSetupModel.checklistItemList[%{#listStatus.index}].checklistFileModelList[0].file_id" value="%{#checklistItem.checklistFileModelList[0].file_id}" />
                                    ...
                                    <span class="checklist-file-date text-truncate max-w-200"><s:property value="%{#checklistItem.checklistFileModelList[0].created_date_str}" /></span>
                        ...
                        <td class="col-md-2">
                            <div class="form-check">
                                <input class="u10checklistform u20checklistform form-check-input" id="cl_result_c_<s:property value='%{#checklistItem.id}' />" type="radio" name="model.u20ChecklistSetupModel.checklistItemList[<s:property value='%{#listStatus.index}' />].checklistResultList[0].cl_result" value="C" <s:if test='%{#checklistItem.checklistResultList[0].cl_result.equals("C")}'>Checked</s:if> />
                            ...
                            <div class="form-check">
                                <input class="u10checklistform u20checklistform form-check-input" id="cl_result_e_<s:property value='%{#checklistItem.id}' />" type="radio" name="model.u20ChecklistSetupModel.checklistItemList[<s:property value='%{#listStatus.index}' />].checklistResultList[0].cl_result" value="E" <s:if test='%{#checklistItem.checklistResultList[0].cl_result.equals("E")}'>Checked</s:if> />
                            ...
                            <div class="form-check">
                                <input class="u10checklistform u20checklistform form-check-input" id="cl_result_n_<s:property value='%{#checklistItem.id}' />" type="radio" name="model.u20ChecklistSetupModel.checklistItemList[<s:property value='%{#listStatus.index}' />].checklistResultList[0].cl_result" value="N" 
                                    <s:if test='%{#checklistItem.checklistResultList[0].cl_result.length > 0}'>
                                        <s:if test='%{#checklistItem.checklistResultList[0].cl_result.equals("N")}'>Checked</s:if> 
                                    </s:if>
                                    <s:else>
                                        <s:if test='%{#checklistItem.checklistFileModelList.size() > 0}'>
                                            <s:if test='%{#checklistItem.checklistResultList[0].cl_result.equals("N")}'>Checked</s:if> 
                                        </s:if>
                        ...
                        <td class="col-md-5">
                            <s:if test='#checklistItem.ci_datatype.equals("PFR")'>
                                <s:textarea theme="simple" name="model.u20ChecklistSetupModel.checklistItemList[%{#listStatus.index}].checklistResultList[0].cl_remarks" value="%{paymentRemarks}" cssClass="form-control" rows="5" maxlength="1000" readonly="true"  />
                            </s:if>
                            <s:else>
                                <s:textarea theme="simple" name="model.u20ChecklistSetupModel.checklistItemList[%{#listStatus.index}].checklistResultList[0].cl_remarks" value="%{#checklistItem.checklistResultList[0].cl_remarks}" cssClass="form-control" rows="5" maxlength="1000"/>
                            </s:else>
                        </td>
                            ...
                            <s:if test="#checklistItem.checklistResultList[0].checklistSupportFileList.size() > 0">
                                <s:iterator value = "#checklistItem.checklistResultList[0].checklistSupportFileList" status="supportFileStatus" var="supportFile">
                                    ...
                                    <s:include value="/base/uppyIncludeSingleFile_utimaps.jsp">
                                        ...
                                        <s:param name="checklist_type">U20</s:param>
                                    </s:include>
                                </s:iterator>
                            </s:if>
                            <s:else>
...
<h5 class="p-1 border-bottom border-bottom-lg-1 bg-secondary text-white mb-3">Other miscellaneous supporting document</h5>
<s:if test="model.getSMS.size() > 0">
    <s:iterator value="model.getSMS" status="smsStatus" var="smsItem">
        <div class="d-flex mt-1">
            <div class="col-md-1">
                <div class="ms-2 circle-div text-uppercase">
                    <s:if test="#smsItem.file_ext != ''"><s:property value="#smsItem.file_ext" /></s:if><s:else>-</s:else>
                </div>
            </div>
            <div class="col-md-11">
                <p class="checklist-file ">
                    <a class="color-file-<s:property value="#smsItem.file_status" />" href="viewTempFileAttachment?uploadID=<s:property value="#smsItem.file_id" />" target="_blank">
                        <s:property value="#smsItem.file_name" />
                    </a>
                </p>
                <span class="checklist-file-date"><s:property value="#smsItem.created_date_str" /></span>
                <span class="checklist-file-size mb-3"><s:property value="#smsItem.file_size" /></span>
            </div>
        </div>
    </s:iterator>
</s:if>
