<%-- 
    Document   : letter_uscs20
    Created on : May 31, 2024, 9:31:25 AM
    Author     : yonglai
--%>
...
<div class="accordion-item">
    <h2 class="accordion-header" id="flush-headingUSCS20">
        <button class="accordion-button <s:if test='!activeAccordion.equals("3")'>collapsed</s:if> fw-bold text-uppercase" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseUSCS20" aria-expanded="false" aria-controls="flush-collapseUSCS20">
            <i class="fas fa-tasks me-2"></i><s:text name = "utimaps.form.submission.uscs20" />
        </button>
    </h2>

    <div id="flush-collapseUSCS20" class="accordion-collapse collapse <s:if test='activeAccordion.equals("3")'>show</s:if>" aria-labelledby="flush-headingUSCS20" data-bs-parent="#accordionUtitliyChecklist">
            <div class="accordion-body">
            <s:if test='fileID==NULL'>
                <IFRAME id="pdfviewer" src="pdfViewerSubmission?pJobId=${model.job_id}&pType=USCS20&cType=${checklistType}&output=embed"  WIDTH=100% HEIGHT=500  align="center" >
                </IFRAME>
            </s:if><s:else>
            ...
            <br><br>
            <form action="" method="post" id="USCS20Form" enctype="multipart/form-data">
                ...
                <s:hidden name="checklistType" value="%{checklistType}"/> 
                <s:hidden name="processType" id="processType" value="152"/> 
                ...
                <s:if test='(!isSigned(model.job_id, "USCS20")) && model.wf_status.equals("152")'>
                ...
                <div class="row">
                    <div class="col-md-12 text-end mb-3">
                        <s:if test='rightToUpdate && model.wf_status.equals("152")'>
                            <s:if test='(!isSigned(model.job_id, "USCS20")) && model.wf_status.equals("152")'>
                            ...
                            <s:if test='isSigned(model.job_id, "USCS20") && model.wf_status.equals("152")'>
                                ...
                                <button class="btn btn-success" id="completeUSCS20Submission" type="submit"><i class="fas fa-check"></i> <s:text name = "utimaps.form.button.complete" /></button>
                            </s:if>
                        </s:if>
                    </div>
                </div>
...
<script nonce="r4DjhKbfO5ry">
    $(document).ready(function () {
        scrollToView("flush-collapseUSCS20");
    });
    ...
    $('#signLetterSubmission').click(function (e) {
        ...
        var status__ = validateForm_bshor('USCS20Form');
        if (status__) {
            var displayMessage = "<s:text name="utimaps.form.message.signLetter" />";

            confirmationBox(displayMessage, "#USCS20Form", "pdfSignSubmission");
        } else {
            showErrors(document.getElementById('USCS90Form'));
        }
    });

    $('#completeUSCS20Submission').click(function (e) {
        e.preventDefault();
        var displayMessage = "<s:text name='utimaps.job.complete' />";

        confirmationBox(displayMessage, "#USCS20Form", "processCompleteJobSubmission");
    });

    $('#revokeUSCSLetter').click(function (e) {
        e.preventDefault();
        var displayMessage = "You are about to recall the signed letter.";

        confirmationBox(displayMessage, "#USCS20Form", "pdfRevokeSubmission");
    });
