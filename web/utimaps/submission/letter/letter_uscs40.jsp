...
<div class="accordion-item">
    <h2 class="accordion-header" id="flush-headingUSCS50">
        <button class="accordion-button <s:if test='!activeAccordion.equals("3")'>collapsed</s:if> fw-bold text-uppercase" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseUSCS50" aria-expanded="false" aria-controls="flush-collapseUSCS50">
            <i class="fas fa-tasks me-2"></i><s:text name = "utimaps.form.submission.uscs40" />
        </button>
    </h2>

    <div id="flush-collapseUSCS50" class="accordion-collapse collapse <s:if test='activeAccordion.equals("3")'>show</s:if>" aria-labelledby="flush-headingUSCS50" data-bs-parent="#accordionUtitliyChecklist">
            <div class="accordion-body">
            <s:if test='fileID==NULL'>
                <IFRAME id="pdfviewer" src="pdfViewerSubmission?pJobId=${model.job_id}&pType=USCS40&output=embed"  WIDTH=100% HEIGHT=500  align="center" >
                </IFRAME>
            </s:if><s:else>
            ...
            <br><br>
            <form action="" method="post" id="USCS40Form" enctype="multipart/form-data">
                ...
                <s:hidden name="processType" id="processType" value="212"/> 
                ...
                <s:if test='(!isSigned(model.job_id, "USCS40")) && model.wf_status_2.equals("212")'>
                    ...
                    <div class="col-md-12 text-end mb-3">
                        <s:if test="rightToUpdate">
                            ...
                            <s:if test='isSigned(model.job_id, "USCS40") && model.wf_status_2.equals("212")'>
                                ...
                                <button class="btn btn-success" id="completeUSCS40Submission" type="submit"><i class="fas fa-check"></i> <s:text name = "utimaps.form.button.complete" /></button>
                            </s:if>
                            <s:if test='model.wf_status_2.equals("210")'>
                                <button class="btn btn-success" id="completeUSCS40Submission" type="submit"><i class="fas fa-check"></i> <s:text name = "utimaps.form.button.complete" /></button>
                            </s:if>
                        </s:if>
                    </div>
...
<script nonce="r4DjhKbfO5ry">
    $(document).ready(function () {
        scrollToView("flush-collapseUSCS40");
    });
    ...
    $('#signLetterSubmission').click(function (e) {
        ...
        var status__ = validateForm_bshor('USCS40Form');
        if (status__) {
            var displayMessage = "<s:text name="utimaps.form.message.signLetter" />";

            confirmationBox(displayMessage, "#USCS40Form", "pdfSignSubmission");
        } else {
            showErrors(document.getElementById('USCS90Form'));
        }
    });

    $('#completeUSCS40Submission').click(function (e) {
        e.preventDefault();
        var displayMessage = "<s:text name='utimaps.job.complete' />";

        confirmationBox(displayMessage, "#USCS40Form", "processComplete2JobSubmission");
    });
    ...
    $('#revokeUSCSLetter').click(function (e) {
        ...
        confirmationBox(displayMessage, "#USCS40Form", "pdfRevokeSubmission");
    });
