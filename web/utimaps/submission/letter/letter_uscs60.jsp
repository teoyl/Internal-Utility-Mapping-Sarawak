...
<div class="accordion-item">
    <h2 class="accordion-header" id="flush-headingUSCS60">
        <button class="accordion-button <s:if test='!activeAccordion.equals("3")'>collapsed</s:if> fw-bold text-uppercase" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseUSCS60" aria-expanded="false" aria-controls="flush-collapseUSCS60">
            <i class="fas fa-tasks me-2"></i><s:text name = "utimaps.form.submission.uscs60" />
        </button>
    </h2>
    ...
    <div id="flush-collapseUSCS60" class="accordion-collapse collapse <s:if test='activeAccordion.equals("3")'>show</s:if>" aria-labelledby="flush-headingUSCS60" data-bs-parent="#accordionUtitliyChecklist">
            ...
            <s:if test='fileID==NULL'>
                <IFRAME id="pdfviewer" src="pdfViewerSubmission?pJobId=${model.job_id}&pType=USCS60&output=embed"  WIDTH=100% HEIGHT=500  align="center" >
                </IFRAME>
            </s:if><s:else>
            ...
            <br><br>
            <form action="" method="post" id="USCS60Form" enctype="multipart/form-data">
                ...
                <s:hidden name="processType" id="processType" value="250"/> 
                <s:hidden name="model.wf_status" id="model.wf_status" value="%{model.wf_status}"/> 
                <s:hidden name="model.wf_status_2" id="model.wf_status_2" value="250"/> 
                ...
                <s:if test='!isSigned(model.job_id, "USCS60")'>
                ...
                <div class="row">
                    <div class="col-md-12 text-end mb-3">
                        ...
                        <s:if test='isSigned(model.job_id, "USCS60")'>
                            ...
                            <button class="btn btn-success" id="completeUSCS60Submission" type="submit"><i class="fas fa-check"></i> <s:text name = "utimaps.form.button.complete" /></button>
                        </s:if>
                    </div>
                </div>
            </form>
...
<script nonce="r4DjhKbfO5ry">
    $(document).ready(function () {
        scrollToView("flush-collapseUSCS60");
    });
    ...
    $('#signLetterSubmission').click(function (e) {
        ...
        var status__ = validateForm_bshor('USCS60Form');
        if (status__) {
            var displayMessage = "<s:text name="utimaps.form.message.signLetter" />";

            confirmationBox(displayMessage, "#USCS60Form", "pdfSignSubmission");
        } else {
            showErrors(document.getElementById('USCS90Form'));
        }
    });

    $('#completeUSCS60Submission').click(function (e) {
        e.preventDefault();
        var displayMessage = "<s:text name='utimaps.job.complete' />";

        confirmationBox(displayMessage, "#USCS60Form", "processComplete2JobSubmission");
    });
    ...
    $('#revokeUSCSLetter').click(function (e) {
        ...
        confirmationBox(displayMessage, "#USCS60Form", "pdfRevokeSubmission");
    });
