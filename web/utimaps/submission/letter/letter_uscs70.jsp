<%-- 
    Document   : letter_uscs70
    Created on : Jun 26, 2024, 8:23:44 AM
    Author     : yonglai
--%>
...
<div class="accordion-item">
    <h2 class="accordion-header" id="flush-headingUSCS70">
        <button class="accordion-button <s:if test='!activeAccordion.equals("3")'>collapsed</s:if> fw-bold text-uppercase" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseUSCS70" aria-expanded="false" aria-controls="flush-collapseUSCS70">
            <i class="fas fa-tasks me-2"></i><s:text name = "utimaps.form.submission.uscs70" />
        </button>
    </h2>

    <div id="flush-collapseUSCS70" class="accordion-collapse collapse <s:if test='activeAccordion.equals("3")'>show</s:if>" aria-labelledby="flush-headingUSCS70" data-bs-parent="#accordionUtitliyChecklist">
            <div class="accordion-body">
            <s:if test='fileID==NULL'>
                <IFRAME id="pdfviewer" src="pdfViewerSubmission?pJobId=${model.job_id}&pType=USCS70&output=embed"  WIDTH=100% HEIGHT=500  align="center" >
                </IFRAME>
            </s:if><s:else>
            ...
            <br><br>
            <form action="" method="post" id="USCS70Form" enctype="multipart/form-data">
                ...
                <s:hidden name="processType" id="processType" value="270"/> 
                ...
                <s:if test='!isSigned(model.job_id, "USCS70") && model.wf_status_2.equals("270")'>
                ...
                <div class="row">
                    <div class="col-md-12 text-end mb-3">
                        ...
                        <s:if test='isSigned(model.job_id, "USCS70") && model.wf_status_2.equals("270")'>
                            ...
                            <button class="btn btn-success" id="completeUSCS70Submission" type="submit"><i class="fas fa-check"></i> <s:text name = "utimaps.form.button.complete" /></button>
                        </s:if>
                    </div>
                </div>
            </form>
...
<script nonce="r4DjhKbfO5ry">
    $(document).ready(function () {
        ...
        if(wfCode === "USJ003_06_02" && wfStatus2 !== "270") {
            // 01/10/2024 :: Testing In Progress
            alertUserOnExistingJob();
        }
        
        scrollToView("flush-collapseUSCS70");
    });
    ...
    $('#signLetterSubmission').click(function (e) {
        ...
        var status__ = validateForm_bshor('USCS70Form');
        if (status__) {
            var displayMessage = "<s:text name="utimaps.form.message.signLetter" />";

            confirmationBox(displayMessage, "#USCS70Form", "pdfSignSubmission");
        } else {
            showErrors(document.getElementById('USCS90Form'));
        }
    });

    $('#completeUSCS70Submission').click(function (e) {
        e.preventDefault();
        var displayMessage = "<s:text name='utimaps.job.complete' />";

        confirmationBox(displayMessage, "#USCS70Form", "processComplete2JobSubmission");
    });
    ...
    $('#revokeUSCSLetter').click(function (e) {
        ...
        confirmationBox(displayMessage, "#USCS70Form", "pdfRevokeSubmission");
    });
