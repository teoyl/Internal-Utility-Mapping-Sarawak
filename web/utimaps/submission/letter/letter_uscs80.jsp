...
<div class="accordion-item">
    <h2 class="accordion-header" id="flush-headingUSCS80">
        <button class="accordion-button <s:if test='!activeAccordion.equals("5")'>collapsed</s:if> fw-bold text-uppercase" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseUSCS80" aria-expanded="false" aria-controls="flush-collapseUSCS80">
            <i class="fas fa-tasks me-2"></i><s:text name = "utimaps.form.submission.uscs80" />
        </button>
    </h2>

    <div id="flush-collapseUSCS80" class="accordion-collapse collapse <s:if test='activeAccordion.equals("5")'>show</s:if>" aria-labelledby="flush-headingUSCS80" data-bs-parent="#accordionUtitliyChecklist">
            <div class="accordion-body">
            <s:if test='fileID==NULL'>
                <IFRAME id="pdfviewer" src="pdfViewerSubmission?pJobId=${model.job_id}&pType=USCS80&cType=${checklistType}&output=embed"  WIDTH=100% HEIGHT=500  align="center" >\</IFRAME>
            </s:if><s:else>
                <IFRAME id="pdfviewer" src="viewTempFileSubmission?fileID=${fileID}&output=embed"  WIDTH=100% HEIGHT=500  align="center" ></IFRAME>
            </s:else>
            <br><br>
            <form action="" method="post" id="USCS80Form" enctype="multipart/form-data">
                ...
                <s:hidden name="processType" id="processType" value="290"/> 
                ...
                <s:if test='!isSigned(model.job_id, "USCS80") && model.wf_status_2.equals("290")'>
                ...
                <div class="row">
                    <div class="col-md-12 text-end mb-3">
                        <s:if test="rightToUpdate">
                            <s:if test='(!isSigned(model.job_id, "USCS80")) && (model.wf_status_2.equals("290") || model.wf_status_2.equals("340"))'>
                            ...
                            <s:if test='isSigned(model.job_id, "USCS80") && (model.wf_status_2.equals("290") || model.wf_status_2.equals("340"))'>
                                ...
                                <button class="btn btn-success" id="completeUSCS80Submission" type="submit"><i class="fas fa-check"></i> <s:text name = "utimaps.form.button.complete" /></button>
                            </s:if>
                        </s:if>
                    </div>
                </div>
...
<script nonce="r4DjhKbfO5ry">
    $(document).ready(function () {
        scrollToView("flush-collapseUSCS80");
    });
    ...
    $('#signLetterSubmission').click(function (e) {
        ...
        var status__ = validateForm_bshor('USCS80Form');
        if (status__) {
    ...
    $('#completeUSCS80Submission').click(function (e) {
        e.preventDefault();
        var canProceed = true;
